...
JSONファイルを以下のような４次元のリストに変換せよ．

```data[h][i][j][k]```

ここで，hはフレーム番号，iは人のID(0または1)，jは関節番号(0~24)，kはx座標(0)，y座標(1)，信頼度(2)を表す．
...
<!--
## 課題7
[sample.mp4](sample.mp4)をYolov8を用いて物体認識を行い，認識された物体を重複を除いて列挙せよ．Yolov8のインストール，利用法は[ここ](yolov8.pdf)を参照すること．物体認識がうまく動作しない場合は，認識済みの[labels.zip](labels.zip)を用いても良い．
